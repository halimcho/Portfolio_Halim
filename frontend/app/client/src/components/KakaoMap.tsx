...
type Lang = "ko" | "en";
type KakaoMapProps = {
  language?: Lang; lat?: number; lng?: number; level?: number;
  draggable?: boolean; scrollwheel?: boolean;
  onAddressChange?: (address: string) => void;
  showZoomControl?: boolean; showMapTypeControl?: boolean; enableGeolocate?: boolean;
  enableSearch?: boolean; searchPlaceholder?: string; myLocationIconUrl?: string;
};
...
const TEXT = {
  ko:{search:"검색",searchPlaceholder:"검색",basis_me:"내 위치 기준 검색",basis_map:"지도 중심 기준 검색",
      dist_me:"거리: 내 위치",dist_map:"거리: 지도 중심",radius:(km:number)=>`반경 ${km}km`,radiusNation:"전국 전체",
      sort_acc:"정확도순",sort_dist:"거리순",noResult:"검색 결과가 없습니다.",results:(n:number)=>`결과 ${n}개`,
      page:(c:number,l:number)=>`페이지 ${c} / ${l}`,prev:"이전",next:"다음",close:"닫기",
      goHere:"여기로 이동",viewDetail:"상세보기",distance:"거리",loadingMap:"지도를 불러오는 중…",
      myLocBtn:"내 위치",myLocToastFail:"현재 위치를 가져올 수 없습니다. 브라우저 권한을 확인해주세요.",
      tooltipMyLoc:"내 위치로 이동",map:"지도",sky:"스카이뷰",zoomIn:"확대",zoomOut:"축소"},
  en:{search:"Search",searchPlaceholder:"Search places",basis_me:"Search from My Location",basis_map:"Search from Map Center",
      dist_me:"Distance: My location",dist_map:"Distance: Map center",radius:(km:number)=>`Radius ${km} km`,radiusNation:"Nationwide",
      sort_acc:"Accuracy",sort_dist:"Distance",noResult:"No results found.",results:(n:number)=>`Results ${n}`,
      page:(c:number,l:number)=>`Page ${c} / ${l}`,prev:"Prev",next:"Next",close:"Close",
      goHere:"Go here",viewDetail:"Details",distance:"Distance",loadingMap:"Loading map…",
      myLocBtn:"My location",myLocToastFail:"Unable to get your current location. Please check browser permission.",
      tooltipMyLoc:"Go to my location",map:"Map",sky:"Skyview",zoomIn:"Zoom in",zoomOut:"Zoom out"}
} as const;

export default function KakaoMap({
  language="ko", lat=37.5665, lng=126.978, level=3, draggable=true, scrollwheel=true, onAddressChange,
  showZoomControl=true, showMapTypeControl=true, enableGeolocate=true,
  enableSearch=true, searchPlaceholder, myLocationIconUrl,
}: KakaoMapProps){
  ...
  const [useMyLocationBias,setUseMyLocationBias]=useState(true); // true=내 위치, false=뷰포트
  const [distanceAnchor,setDistanceAnchor]=useState<"me"|"map">("me");
  const [radiusKm,setRadiusKm]=useState<number>(0); // 0=전국
      ...
      <div style="min-width:240px;max-width:320px;padding:10px;border-radius:12px;background:${bg};
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button id="km-close" style="padding:6px 10px;border-radius:8px;background:transparent;
                  border:1px solid ${bd};color:${fg};font-size:12px;cursor:pointer">${L.close}</button>
        </div>
      </div>`;
    const overlay=new kakao.maps.CustomOverlay({position:ll,content:html,xAnchor:0.5,yAnchor:1.1,zIndex:9999});
    overlay.setMap(mapObjRef.current); overlayRef.current=overlay;
    setTimeout(()=>{ (overlay.getContent() as HTMLElement)
      .querySelector<HTMLButtonElement>("#km-close")
      ?.addEventListener("click",()=>{overlay.setMap(null);overlayRef.current=null;}); },0);
  };
  ...
  useEffect(()=>{
    ...
    const onEnd=()=>{ if(zoomTimer.current) cancelAnimationFrame(zoomTimer.current);
    ...
    clustererRef.current=new kakao.maps.MarkerClusterer({map,averageCenter:true,minLevel:6,disableClickZoom:false});
    ...
    kakao.maps.event.addListener(map,"click",async (e:any)=>{
      ...
      const radii=[50,120,300,600]; 
      let candidate:any|null=null;
      for(const r of radii){
        const found=await new Promise<any|null>(resolve=>{
          places.keywordSearch(query.trim() || " ",
            ...
            {location:ll,radius:r,size:15,sort:kakao.maps.services.SortBy.DISTANCE}
          );
        });
      ...
      if(candidate){
        const anchor={lat:ll.getLat(),lng:ll.getLng()};
        const d=haversine(anchor,{lat:Number(candidate.y),lng:Number(candidate.x)});
        if(d<=100){ openOverlay(candidate); } else candidate=null;
      }
      if(!candidate){
        try{
          const geocoder=new kakao.maps.services.Geocoder();
          geocoder.coord2Address(ll.getLng(),ll.getLat(),(res:any,stat:string)=>{
            const addr=stat===kakao.maps.services.Status.OK
              ? (res?.[0]?.road_address?.address_name || res?.[0]?.address?.address_name || "")
              : "";
            openAddressOverlay(addr,ll);
          });
        }catch{ openAddressOverlay("",ll); }
      }
  ...
  const openOverlay=(place:any)=>{
    ...
    overlay.setMap(mapObjRef.current); overlayRef.current=overlay;
    setTimeout(()=>{
      const c=overlay.getContent() as HTMLElement;
      c.querySelector<HTMLButtonElement>("#km-close")?.addEventListener("click",()=>{overlay.setMap(null);overlayRef.current=null;});
      c.querySelector<HTMLButtonElement>("#km-go")?.addEventListener("click",()=>{mapObjRef.current.panTo(pos);});
    },0);
  };
  ...
  const handleSearch=(page?:number)=>{
    ...
    if(order==="distance" && radiusKm!==0) opts.sort=kakao.maps.services.SortBy.DISTANCE;

    if(radiusKm===0){
    }else if(useMyLocationBias && myMarkerRef.current){
      opts.location = myMarkerRef.current.getPosition();
      opts.radius   = radiusKm*1000;
    }else{
      opts.useMapBounds = true; 
    }
    ...
    places.keywordSearch(
      ...
      (data:any[],status:string,pagination:any)=>{
        if(status!==kakao.maps.services.Status.OK){
          setResults([]); setPageInfo({current:1,last:1}); clearResultsMarkers(); alert(L.noResult); return;
        }
        ...
        data.forEach((place)=>{
          ...
          newMarkers.push(marker); resultsMarkersRef.current.push(marker); bounds.extend(pos);
        });
        clustererRef.current.addMarkers(newMarkers);
        if(radiusKm>0 && !opts.useMapBounds && !bounds.isEmpty()) mapObjRef.current.setBounds(bounds);
  ...
  const radiusOptions=useMemo(()=>[0,1,2,3,5,10,15,20],[]);
  ...
  return (
    <div className="w-full relative">
      ...
      <div
        ...
        aria-live="polite" aria-busy={loading ? "true" : undefined}
      />
        ...
        <>
          <div className="absolute top-3 right-3 z-20">
            <div className={`${cardCls} ${cardTone} p-1 flex rounded-lg`}>
              <button onClick={()=>setType("ROADMAP")}
                className={`px-3 py-1.5 text-sm rounded-md ${mapType==="ROADMAP"?"bg-blue-600 text-white":""}`} title={L.map}>{L.map}</button>
              <button onClick={()=>setType("SKYVIEW")}
                className={`ml-1 px-3 py-1.5 text-sm rounded-md ${mapType==="SKYVIEW"?"bg-blue-600 text-white":""}`} title={L.sky}>{L.sky}</button>
            </div>
          </div>
      ...
      {enableGeolocate && (
        <button type="button" onClick={goMyLocation}
          className="absolute bottom-4 right-4 z-20 rounded-full bg-white/95 text-slate-700 border border-slate-300
                     px-4 py-2 text-sm font-medium shadow-lg hover:bg-white dark:bg-slate-800/90 dark:text-slate-100 dark:border-slate-600
                     focus:outline-none focus:ring-2 focus:ring-blue-500"
          title={L.tooltipMyLoc}>📍 {L.myLocBtn}</button>
      )}
            ...
            <div className="flex flex-wrap gap-3 items-stretch relative">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <button onClick={()=>handleSearch(1)}
                  className="absolute right-1 top-1/2 -translate-y-1/2 h-9 px-3 rounded-md bg-blue-600 text-white text-[13px] shadow hover:bg-blue-700">
                  {L.search}
                </button>
                ...
                {showHistory && recentQueries.length>0 && (
                  <div className={`absolute left-0 right-0 mt-1 max-h-60 overflow-auto z-30 ${cardCls} ${cardTone}`}>
                    <ul className="py-1 text-[13px]">
                      {recentQueries.map((q)=>(
                        <li key={q} className="px-3 py-1.5 flex items-center justify-between gap-3 hover:bg-slate-100 dark:hover:bg-slate-800/60"
                            onMouseDown={(e)=>e.preventDefault()}>
                          <button className="flex-1 text-left truncate" onClick={()=>{ setQuery(q); setShowHistory(false); handleSearch(1); }}>
                            {q}
                          </button>
                          <button aria-label={`remove ${q}`}
                            className="shrink-0 inline-flex items-center justify-center rounded-md w-6 h-6 text-slate-500 hover:text-slate-700 hover:bg-slate-200
                                       dark:text-slate-400 dark:hover:text-slate-100 dark:hover:bg-slate-700/70"
                            onMouseDown={(e)=>e.preventDefault()}
                            onClick={()=> setRecentQueries(prev=>prev.filter(item=>item!==q))}>✕</button>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <select
                ...
                title={useMyLocationBias ? L.basis_me : L.basis_map}
                className="h-10 rounded-lg border px-3 text-[13px] bg-white/70 border-slate-300 text-slate-800
              ...
              <select
                ...
                title={radiusKm===0 ? L.radiusNation : L.radius(radiusKm)}
                className="h-10 rounded-lg border px-3 text-[13px] bg-white/70 border-slate-300 text-slate-800
                           dark:bg-slate-800/70 dark:border-slate-600 dark:text-slate-100"
              >
                <option value={0}>{L.radiusNation}</option>
                {radiusOptions.filter(k=>k!==0).map((km)=>(<option key={km} value={km}>{L.radius(km)}</option>))}
              </select>
        ...
        <div className="absolute left-3 right-3 bottom-3 md:left-5 md:right-5 z-20">
          <div className={`${cardCls} ${cardTone} overflow-hidden`}>
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-slate-200 dark:border-slate-700">
              <div className="text-[12px]">{L.results(results.length)} • {L.page(pageInfo.current,pageInfo.last)}</div>
              <div className="flex items-center gap-1.5">
                <button disabled={pageInfo.current<=1} onClick={()=>handleSearch(pageInfo.current-1)}
                        className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-300 text-slate-800 text-[12px] disabled:opacity-40
                                   dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100">{L.prev}</button>
                <button disabled={pageInfo.current>=pageInfo.last} onClick={()=>handleSearch(pageInfo.current+1)}
                        className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-300 text-slate-800 text-[12px] disabled:opacity-40
                                   dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100">{L.next}</button>
                <button onClick={()=>setShowPanel(false)}
                        className="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-300 text-slate-800 text-[12px]
                                   dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100">{L.close}</button>
              </div>
            </div>
      ...
      {loading && (
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400" role="status" aria-live="polite">{L.loadingMap}</p>
      )}
    </div>
  );
}
